...
from .utils import convert_lat_lon, filter_ds, save_plot, write_to_s3


def generate_height_and_wind_images(hour, grib_file, today, model_run, local_run):
    extent = [-160, -40, 5, 75]
    for level in ["200", "300", "500", "700", "850"]:
        ...
        filepath = save_plot(plt, model_run, hour, level=f"{level}mb", plot_type="height_wind")

        if local_run is False:
            write_to_s3(filepath, today, model_run, f"{level}mb", "height_wind")
