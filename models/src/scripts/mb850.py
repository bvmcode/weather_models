...
def generate_dew_point_images(hours, grib_files, model_run):
    for i, (hour, grib_file) in enumerate(zip(hours, grib_files)):
        ...
        filepath = f"{os.getcwd()}/images/850mb/dewpoint/{today}/{model_run}/{i}_hour_{hour}.png"
...
def generate_rh_images(hours, grib_files, model_run):
    for i, (hour, grib_file) in enumerate(zip(hours, grib_files)):
        print(f"Processing: {grib_file}")
        ...
        # Save image
        today = datetime.now().strftime("%Y%m%d")
        filepath = f"{os.getcwd()}/images/850mb/rh_wind/{today}/{model_run}/{i}_hour_{hour}.png"
